...
$sql = "SELECT * FROM nav_admin WHERE ad_email = :email";
...
if ($user) {
    // Verify the password
    if (password_verify($inputPassword, $user['ad_password'])) {
        echo json_encode(['status' => 'success', 'message' => 'Login successful', 'username' => $user['ad_name'], 'id' => $user['ad_id'] ]);
    } else {
